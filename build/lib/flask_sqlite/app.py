...
def connect():
    return sqlite3.connect(os.path.join(os.path.dirname(__file__), 'db', 'data.db'))
...
def insert():
    ...
    try:
        data.to_sql(name=filename, con=con, index=False)
    except:
        pass
...
def compare():
    ...
    files = [k for k,v in request.form.items() if v == 'on']
    custom = [custom_dfs[k] for k in files if custom_dfs.get(k, None) is not None]
    files = [read(k) for k in files if custom_dfs.get(k, None) is None]
    files += custom
...
def query():
    ...
    if name in files or ' ' in name or not name:
            operation = "Save Fail"
    else:
